...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="page" mode="carousel">
		...
		<xsl:param name="recommendId" />
		
		<li>
			<div class="img">
				<a href="/categories/{$limiter}?start_good={@id}&amp;recommend_to={$recommendId}">
			...
			<div class="bottom">
				...
				<div class="buy"><a href="/categories/{$limiter}?start_good={@id}&amp;recommend_to={$recommendId}"></a></div>
	...
	<xsl:template name="carouselImg">
		...
		<xsl:variable name="thumb" select="document(concat('udata://system/makeThumbnailFull/(.', $img, ')/240/240'))//src" />
		...
		<img src="{$exactImg}" width="140" alt="{$header}" />
	</xsl:template>
