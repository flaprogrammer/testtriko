...
$(document).ready(function()
{
	...
	setTimeout(function(){make_scroll_carousel();}, 500);
	...
	/*
	$('#brendMashuk').click(function(){
		if($('#brendBossa input').attr("checked") != 'checked') {
			return false;
		} else {
			$('#brendMashuk input').removeAttr("checked");
		}
	});
	$('#brendBossa').click(function(){
		if($('#brendMashuk input').attr("checked") != 'checked') {
			return false;
		} else {
...
function makeLightbox()
{
	...
	var indexPhoto = 0;  // индекс фотографии
        ...
        $('span.get-image').click(function(){
            $('table#goodSizes span.get-image').removeClass('active-color');
            $('table#goodSizes span.arrow-blue').css('display', 'none');
            $(this).parents('tr').find('span.arrow-blue').css('display', 'block');
            $(this).addClass('active-color');
            var idColor = $(this).attr('id');
            $('#goodSmallImages a#'+idColor).click();
        });
        $('span.get-image').removeClass('activeColor');
        $('a.sizeColors').click(function(){
            $('table#goodSizes span.get-image').removeClass('active-color');
            $('table#goodSizes span.arrow-blue').css('display', 'none');
            $(this).parents('tr').find('span.arrow-blue').css('display', 'block');
            $(this).parents('tr').find('span.get-image').addClass('active-color');
            var idColor = $(this).parents('tr').find('span.get-image').attr('id');

            $('#goodSmallImages a#'+idColor).click();
        });
        $('td.amount-color input').click(function(){
            $('table#goodSizes span.get-image').removeClass('active-color');
            $('table#goodSizes span.arrow-blue').css('display', 'none');
            $(this).parents('tr').find('span.arrow-blue').css('display', 'block');
            $(this).parents('tr').find('span.get-image').addClass('active-color');
        ...
        if($('#goodBigImage').hasClass('colors-size')){
            ...
            $('#goodSmallImages a, #numImage li').bind('click', function(){
                    ...
                    var id = $('#goodSmallImages a').eq(indexPhoto).attr('id');
                    if(id) {
                        $('table#goodSizes span.arrow-blue').css('display', 'none');
                        $('table#goodSizes span.get-image').removeClass('active-color');
                        $('table#goodSizes span#'+id).addClass('active-color');
                        $('table#goodSizes span#'+id).parents('tr').find('span.arrow-blue').css('display', 'block');
                    }
                    $('#numImage li').css('width', '10px');
                    $('#numImage li').css('height', '10px');
                    ...
                    $('#numImage li').eq(indexPhoto).css('width', '14px');
                    $('#numImage li').eq(indexPhoto).css('height', '14px');
		...
		$('#goodSmallImages').show(300);
		clearInterval(timer);
            }, function(){
                    $('#goodSmallImages').hide(300);
                    timer = setInterval(slidePhoto, 8000);
            });
